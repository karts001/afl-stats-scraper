...
class PlayerService():
    ...
    async def insert_players(self, player_dtos: Set[PlayerProfileDTO]) -> None:
        await self.repo.insert_players(player_dtos)

    async def get_player_id(self, display_name: str, dob: str) -> str | None:
        return await self.repo.get_player_id(display_name, dob)

    def check_if_player_in_dto_set(self, display_name, dob, dtos: Set[PlayerProfileDTO]) -> str | None:
        for dto in dtos:
